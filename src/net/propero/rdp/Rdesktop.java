...
public class Rdesktop {
     ...
     * 文件映射分隔符
     * 示例：
     * share@D:\\Download,test@D:\\upload
     * 分别把本地的download和upload在远程桌面中映射为share，upload
     */
    private final static String MUL_DISK_MAP_SPLIT = ",";
    private final static String DISK_DEVICE_MAP_SPLIT = "@";
    ...
    public static void usage() {
        ...
        System.err
                .println("	--disk_device_map=name@path,name@path			map local folder path to remote desktop");
        System.err
                .println("Example: java net.propero.rdp.Rdesktop -g 800x600 -l WARN --disk_device_map share@D:\\Download,test@D:\\upload m52.propero.int");
    ...
    public static void main(String[] args) throws RdesktopException {
        ...
        alo[14] = new LongOpt("persistent_caching", LongOpt.NO_ARGUMENT, null,
                0);
        alo[15] = new LongOpt("disk_device_map", LongOpt.REQUIRED_ARGUMENT, sb,
                0);
            ...
            switch (c) {
                ...
                case 0:
                    switch (g.getLongind()) {
                        ...
                        case 15: {
                            arg = g.getOptarg();
                            logger.info("add Disk Device mapping" + arg);
                            if (null != arg && !arg.isEmpty()) {
                                String[] mapLines = arg.split(MUL_DISK_MAP_SPLIT);
                                if (null != mapLines && mapLines.length > 0) {
                                    for (int i = 0; i < mapLines.length; i++) {
                                        logger.info("process disk device mapping line:" + mapLines[i]);
                                        String[] line = mapLines[i].split(DISK_DEVICE_MAP_SPLIT);
                                        if (null != line && line.length >= 2) {
                                            String name = line[0];
                                            String path = line[1];
                                            logger.info(String.format("map local folder:%s into rdp file:%s", path, name));
                                            diskChannel.addDiskDevice(name, path);
                                        }
                                    }
                                }
                            }
